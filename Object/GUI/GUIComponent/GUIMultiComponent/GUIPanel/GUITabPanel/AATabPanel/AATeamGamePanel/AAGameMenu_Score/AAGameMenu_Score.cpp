...
#include "AA29/Object/GUI/GUIComponent/GUIMultiComponent/GUIPanel/GUITabPanel/AATabPanel/AATeamGamePanel/AAGameMenu_Score/AAGameMenu_Score.h"
#include "AA29/Object/GUI/GUIComponent/GUIMultiComponent/GUITabControl/AAGUITabControl/AAGUITabControl.h"

UAAGameMenu_Score::UAAGameMenu_Score()
{
	...
	c_Tabs = PageTabs;
	PanelClasses = { "AGP_Interface.AATab_AAScoreboard_Summary","AGP_Interface.AATab_AAScoreboard_AssaultTeam","AGP_Interface.AATab_AAScoreboard_DefenseTeam","AGP_Interface.AATab_AAScoreboard_SpectatorTeam" };
	...
	bNeverFocus = true;
	//OnDraw = AAGameMenu_Score.InternalOnDraw;
}

void UAAGameMenu_Score::InitComponent(UGUIController* MyController, UGUIComponent* MyOwner)
{
	/*
	int32 i = 0;
...
void UAAGameMenu_Score::HandleParameters(FString Param1, FString Param2)
{
	/*
	if (Param1 != "")
	{
...
bool UAAGameMenu_Score::GetRestoreParams(FString& Param1, FString& Param2)
{
	/*
...
bool UAAGameMenu_Score::ButtonClicked(UGUIComponent* Sender)
{
	return true;
}

bool UAAGameMenu_Score::InternalOnDraw(UCanvas* C)
{
	/*
	int32 tmpActiveTab = 0;
	int32 i = 0;
	for (i = 0; i < 4; i++)
	{
		if (c_Tabs.TabStack[i].bActive == true)
		{
			tmpActiveTab = i;
		}
	}
	if (LastTeamID != PlayerOwner().PlayerReplicationInfo.Team.TeamIndex)
	{
		...
		{
			c_Tabs.ReplaceTab(c_Tabs.TabStack[1], PanelCaptions[2], PanelClasses[2], , PanelHints[1], true);
			c_Tabs.ReplaceTab(c_Tabs.TabStack[2], PanelCaptions[1], PanelClasses[1], , PanelHints[2], true);
		}
		else
		{
			c_Tabs.ReplaceTab(c_Tabs.TabStack[1], PanelCaptions[1], PanelClasses[1], , PanelHints[1], true);
			c_Tabs.ReplaceTab(c_Tabs.TabStack[2], PanelCaptions[2], PanelClasses[2], , PanelHints[2], true);
		}
		c_Tabs.ActivateTab(c_Tabs.TabStack[tmpActiveTab], true);
		LastTeamID = PlayerOwner().PlayerReplicationInfo.Team.TeamIndex;
	}
	return OnDraw(C);
	*/
...
FString UAAGameMenu_Score::Lower(FString Text)
{
	/*
	int32 IndexChar = 0;
