...
public:
	...
	UPROPERTY(EditAnywhere)				UManifest* SaveGameManifest;							//var() const transient Manifest SaveGameManifest;
	...
	bool NotifyIdlePlayer(AController* C, float LastMovedTime);
	void NotifyIdlePlayerRounds(AController* C, int32 nIdleRounds, bool bMessage);
	bool TestForIdlePlayers(AAA2_PlayerState*  PRI);
	int32 GetTimeElapsed();
	...
	bool IsAuthAvailable(AInternetInfo* ILink);
	...
	bool TournamentWarmupJustEnded();
	...
	FString TournamentServerINIGetTeamName(int32 iTeam);
	...
	FString FindPlayerByID(int32 PlayerID);
	...
	bool BecomeSpectator(APlayerController* p);
	bool AllowBecomeActivePlayer(APlayerController* p);
	...
	bool KickBan(APlayerController* PCAdmin, FString PlayerName, FString time_to_ban_for, FString BanOptions);
	...
	int32 ReduceDamage(int32 Damage, APawn* injured, APawn* instigatedBy, FVector HitLocation, FVector Momentum, UaDamageType* DamageType);
	...
	void RestartGame();
	TArray<FString> GetMapRotation();
	AMapList* GetMapList(FString MapListClassType);
	...
	UMaterialInstance* GetRandomTeamSymbol(int32 Base);
	FString FindTeamDesignation(AAA2_GameState* GRI, AActor* A);
	FString ParseChatPercVar(AMutator* BaseMutator, AController* Who, FString Cmd);
	FString ParseMessageString(AMutator* BaseMutator, AController* Who, FString Message);
