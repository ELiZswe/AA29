...
public:
	...
	AActor* GetActiveWeaponSystem();
	AActor* GetPosition(int32 iPosition);
	FRotator GetWeaponAim();
	...
	float BotDesireability(AActor* S, int32 TeamIndex, AActor* Objective);
	void Destroyed();
	...
	bool CheckTauntValid(FName Sequence);
	bool Occupied();
	...
	AVehicle* OpenPositionFor(APawn* p);
	bool IndependentVehicle();
	AActor* GetBestEntry(APawn* p);
	AVehicle* GetMoveTargetFor(APawn* p);
	...
	int32 NumPassengers();
	APawn* GetInstigator();
	AaAIController* GetBotPassenger();
	bool IsVehicleEmpty();
