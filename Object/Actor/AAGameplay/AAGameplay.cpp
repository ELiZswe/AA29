...
void AAAGameplay::SetAttachments()
{
	...
	for (i=0; i<Attachments.Length; i++)
	{
		...
		{
			Log("AAGameplay::SetAttachments(): ERROR: Couldn't Spawn Item: " $ string(Attachments[i].Attachment) $ " for actor: " $ string(Self));
		}
		else
		{
	...
	for (i=0; i<TagAttachments.Length; i++)
	{
		if (TagAttachments[i].ActorTag != 'None')
		{
			...
			{
				Log("AAGameplay::SetAttachments(): ERROR: Couldn't find actor with tag: " $ string(TagAttachments[i].ActorTag) $ " for actor: " $ string(Self));
			}
			else
			{
			}
			...
			{
				Log("AAGameplay::SetAttachments(): ERROR: Name is None for actor: " $ string(Self));
			}
		}
	}
	*/
