...
UUIRolloverBitmapButton::UUIRolloverBitmapButton()
{
...
void UUIRolloverBitmapButton::Destroyed()
{
	Super::Destroyed();
	_Texture = nullptr;
}
void UUIRolloverBitmapButton::MouseMove(int32 mX, int32 mY)
{
	/*
	if (!_bEnabled)
	{
...
bool UUIRolloverBitmapButton::ProcessKeyEvent(int32 Key, int32 Action, float Delta)
{
	/*
	if (!_bEnabled)
	{
...
void UUIRolloverBitmapButton::Draw(UCanvas* Canvas)
{
	/*
	local int oldstyle;
	if (!_bEnabled)
	...
	if (_Texture != None)
	{
		...
		Canvas.DrawTile(_Texture, _ScaledPos.W, _ScaledPos.H, _iTexDim.X, _iTexDim.Y + _iState * _iTexDim.H, _iTexDim.W, _iTexDim.H);
		if (_bDebug)
		{
			Canvas.SetPos(_ScaledPos.X, _ScaledPos.Y);
...
void UUIRolloverBitmapButton::SetTextureDimensions(FPosition dim)
{
	_iTexDim.X = dim.X;
	_iTexDim.Y = dim.Y;
	_iTexDim.W = dim.W;
...
void UUIRolloverBitmapButton::SetTexture(FString Tex)
{
	if (_Texture == nullptr)
	{
