...
UUICheckboxC::UUICheckboxC()
{

}


void UUICheckboxC::Destroyed()
{
	Super::Destroyed();
	_Texture = nullptr;
}
void UUICheckboxC::MouseMove(int32 mX, int32 mY)
{
	/*
	if (!_bEnabled)
	{
...
bool UUICheckboxC::ProcessKeyEvent(int32 Key, int32 Action, float Delta)
{
	/*
	if (!_bEnabled)
	{
...
void UUICheckboxC::Draw(UCanvas* Canvas)
{
	/*
	local int oldstyle;
	local Font oldfont;
...
void UUICheckboxC::SetTextureDimensions(FPosition dim)
{
	_iTexDim.X = dim.X;
	_iTexDim.Y = dim.Y;
	_iTexDim.W = dim.W;
...
void UUICheckboxC::SetLabel(FString Label)
{
	_sLabel = Label;
}
void UUICheckboxC::SetTexture(FString Tex)
{
	if (_Texture == nullptr)
	{
...
void UUICheckboxC::Select(bool State)
{
	/*
	if (_iGroup != 0 && State)
	{
...
void UUICheckboxC::SetClassName(FString ClassName)
{
	_sClassName = ClassName;
}
void UUICheckboxC::SetPropertyName(FString PropName)
{
	_sPropertyName = PropName;
}
void UUICheckboxC::SetValueName(FString valuename)
{
	_sValueName = valuename;
}
bool UUICheckboxC::IsSelected()
{
	return _bChecked;
}
void UUICheckboxC::SetState(bool Checked)
{
	_bChecked = Checked;
}
void UUICheckboxC::Refresh()
{
	/*
	local string Status;
...
void UUICheckboxC::Accept()
{
	...
	if (cFrame != None && cFrame.cHUD != None && cFrame.cHUD.PlayerOwner != None)
	{
		if (_bChecked)
		{
			PrivateSet(_sClassName $ " " $ _sPropertyName $ " " $ _sValueName);
		}
	}
	*/
}
